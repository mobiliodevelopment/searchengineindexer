...
$urlAPI = "https://searchconsole.googleapis.com/v1/urlInspection/index:inspect?alt=json";

$header[0] = "Authorization: Bearer ".$settings["google-key"];
$header[1] = "Content-Type: application/json; charset=UTF-8";

$ch = curl_init( $urlAPI );
$payload = json_encode( array( "inspectionUrl"=> $url, "siteUrl" => parse_url($url,PHP_URL_SCHEME)."://". parse_url($url,PHP_URL_HOST)."/") ,JSON_UNESCAPED_SLASHES );


//var_dump ($settings["google-key"]);
//var_dump ($payload);
//var_dump ($header);
